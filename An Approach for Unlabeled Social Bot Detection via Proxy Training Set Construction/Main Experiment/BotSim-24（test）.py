...
TARGET_FILE = os.path.join(DATA_DIR, "BotSim-24.csv")           # Pre-extracted target dataset (mapped)

# ====================== Parameters ======================
RANDOM_STATE = 42
BALANCE_THRESHOLD = 0.20
SAMPLE_SIZE_RATIO = 5.0
MIN_SAMPLE_RATIO = 0.75   # If paper requires β=1.5, change to 1.5
MAX_TOL = 0.8
TOL_STEP = 0.02
INIT_TOL = 0.01
TARGET_RATIO = (1, 1)
...
SCALE_FEATURES = ['user_age_days', 'tweet_count', 'avg_tweet_length',
                  'avg_user_mention_count', 'followers_count', 'avg_tweets_per_day']
...
def load_target_data():
    """Load target dataset (BotSim-24) from CSV."""
...
def search_best_combo(X_scaled, feature_names, binary_flags):
    ...
    valid_candidates = []
    # Search 2~4 feature combinations (consistent with paper Appendix G)
    for r in range(2, 5):
...
def align_by_scaling(train_df, target_df, features):
...
def main():
    print("=" * 80)
    print("Main Pipeline (Section 3.2) - Target Domain: BotSim-24 (Feature-Mapped Version)")
    print("Clustering(2~4 features) + Multi-cluster Tolerance Sampling + Batch Global Calibration + Pseudo-alignment")
    print("=" * 80)
    ...
    # 2. Load target data
    print("\n[Loading target data (BotSim-24)]")
    target_df = load_target_data()
    print(f"Target samples: {len(target_df)}")
    print(f"True class distribution: Human {sum(target_df['label'] == 0)}, Bot {sum(target_df['label'] == 1)}")

    # 3. Define common features (mapped feature set)
    common_features = [
        'user_age_days', 'tweet_count', 'avg_tweet_length',
        'description_empty', 'avg_user_mention_count',
        'followers_count', 'avg_hashtag_count', 'avg_tweets_per_day'
    ]
    ...
    if len(common_features) < 3:
        print("Common features less than 3, cannot perform effective clustering, exit.")
        return

    # 4. Compute feature importance (on source domain)
    ...
    # 5. Target domain clustering (search 2~4 feature combinations)
    print("\n[Searching best feature combination on BotSim-24 (2~4 features)]")
    ...
    # 6. Multi-cluster dynamic tolerance sampling
    print("\n[Multi-cluster dynamic tolerance sampling]")
    sample_size = int(len(target_df) * SAMPLE_SIZE_RATIO)
    ...
    # 7. Global class ratio calibration
    print("\n[Global ratio calibration] Adjust to 1:1 bot-human")
    top_k_for_calib = min(5, len(common_features))
    ...
    # 8. Pseudo-alignment
    print("\n[Pseudo-alignment] Scaling numerical features of training set")
    scale_features = []
    ...
    if scale_features:
        train_data_scaled = align_by_scaling(train_data_calibrated, target_df, scale_features)
    else:
        print("  No features to scale, skipping pseudo-alignment")
        train_data_scaled = train_data_calibrated

    # 9. Train and evaluate on BotSim-24
    print("\n[Training and evaluating on BotSim-24]")
